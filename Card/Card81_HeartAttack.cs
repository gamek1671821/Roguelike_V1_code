...
public class Card81_HeartAttack : CardItem, IPointerDownHandler
{
    ...
    {
        if (hitEnemy.data["heart"] == "F")
        {
            damageText.text = $"目標沒有心臟"; //之後可以動態計算傷害
        }
        else
        {
            damageText.text = $"即死"; //之後可以動態計算傷害
    ...
    public override bool TryUse()
    {
        ...
        else if (hitEnemy.data["heart"] == "F")
        {
            //費用不足
            AudioManager.Instance.PlayEffect("Effect/lose"); //使用失敗音效
            UIManager.Instance.showTip("目標沒有心臟", Color.red);
            return false;
        }
        else
        {
