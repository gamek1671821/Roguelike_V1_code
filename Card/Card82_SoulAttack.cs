...
public class Card82_SoulAttack : CardItem, IPointerDownHandler
{
    ...
    {
        if (hitEnemy.data["Soul"] == "F")
        {
            damageText.text = $"目標不是靈體"; //之後可以動態計算傷害
        }
        else
        {
            damageText.text = $"即死"; //之後可以動態計算傷害
    ...
    public override bool TryUse()
    {
        ...
        else if (hitEnemy.data["Soul"] == "F")
        {
            //費用不足
            AudioManager.Instance.PlayEffect("Effect/lose"); //使用失敗音效
            UIManager.Instance.showTip("目標不是靈體", Color.red);
            return false;
        }
        else
        {
