...
public class Card78_PosionAttack : CardItem, IPointerDownHandler
{
    ...
    {
        damageText.text = $"{CountIntellect("Arg0")}傷害 [全體附加{CountPowerPoisoned("Arg1")}劇毒]";
    }
    public override void OnPointDamageText()
    {
        damageText.text = $"{PointMessage(CountIntellect("Arg0"), hitEnemy)}  [全體附加{CountPowerPoisoned("Arg1")}劇毒]"; //之後可以動態計算傷害
    }
    ...
    public override void CardEffect()
    {
        ...
        int val1 = CountPowerPoisoned("Arg1"); //劇毒
        foreach (var enemy in EnemyManager.Instance.enemyList)
        {
            enemy.GetDeBuff(DeBuffType.poisoned, 5, val1);//賦予
            //enemy.Hit(0);
        }
